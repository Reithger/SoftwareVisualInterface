...
public class PopoutSelectList extends PopoutWindow{
	...
	private void drawPage() {
		...
		int size = heiChng / 4;
		if(filtered) {
			this.handleTextEntry(TEXT_ENTRY_FILTER_ACCESS, "no_move", getWidth() / 3, heiChng / 4, getWidth() * 7 / 12, heiChng / 4, CODE_FILTER_ENTRY_ACCESS, null, "");
			...
			this.handleButton("butt_submit", "no_move", getWidth() * 3 / 4, heiChng / 4, size, size, CODE_FILTER_ENTRY_SUBMIT);
		}
		removeElementPrefixed("han");
		for(int i = 0; i < used.length; i++) {
			handleTextButton("han_" + i, "move", getWidth() / 2, (filtered ? heiChng : heiChng / 2) + i * heiChng, getWidth() * 2 / 4, getHeight() / 5, null, used[i], i, Color.gray, Color.black);
		}
		handleText("placeholder", "move", getWidth() / 2, (filtered ? heiChng : heiChng / 2) + (used.length)* heiChng , 10, 10, null, "");
		
		int xPos = getWidth() * 35/40;
		int yPos = getHeight() * 1 / 15;
		
		this.handleTextButton("close", "no_move", xPos,  yPos, size,  size, null, "", CODE_CLOSE, Color.red, Color.black);
