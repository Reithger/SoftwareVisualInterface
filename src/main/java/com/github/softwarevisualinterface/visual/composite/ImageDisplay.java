package com.github.softwarevisualinterface.visual.composite;
...
//---  Operations   ---------------------------------------------------------------------------
	...
	public void drawUI() {
		...
		if(!hideUI) {
			...
			try {
				handleImageButton("ui_box_zoom_in", "no_move", posX - spacing, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("zoom_in.png", ImageDisplay.class.getClassLoader())), CODE_ZOOM_IN);
				handleImageButton("ui_box_zoom_out", "no_move", posX + spacing, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("zoom_out.png", ImageDisplay.class.getClassLoader())), CODE_ZOOM_OUT);
				posY += spacing;
				handleImageButton("ui_box_move_up", "no_move", posX, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("up_arrow.png", ImageDisplay.class.getClassLoader())), CODE_MOVE_UP);
				posY += spacing;
				handleImageButton("ui_box_move_left", "no_move", posX - spacing, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("left_arrow.png", ImageDisplay.class.getClassLoader())), CODE_MOVE_LEFT);
				handleImageButton("ui_box_move_right", "no_move", posX + spacing, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("right_arrow.png", ImageDisplay.class.getClassLoader())), CODE_MOVE_RIGHT);
				handleImageButton("ui_box_UI_ring", "no_move", posX, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("UI_ring.png", ImageDisplay.class.getClassLoader())), CODE_RESET_POSITION);
				posY += spacing;
				handleImageButton("ui_box_move_down", "no_move", posX, posY, imageSize, imageSize, ImageIO.read(IOUtils.resourceToURL("down_arrow.png", ImageDisplay.class.getClassLoader())), CODE_MOVE_DOWN);
		...
		if(!disableToggleUI) {
			try {
				handleImageButton("ui_hide_ui", "no_move", p.getWidth() - 1 * imageSize, imageSize, imageSize * 3 / 2, imageSize * 3 / 2, hideUI ? ImageIO.read(IOUtils.resourceToURL("eye_open-2.png", ImageDisplay.class.getClassLoader())) : ImageIO.read(IOUtils.resourceToURL("eye_closed-2.png", ImageDisplay.class.getClassLoader())), CODE_HIDE_UI);
	...
	public void handleImageButton(String name, String frame, int x, int y, int wid, int hei, Image img, int code) {
		String imageName = name + "_image";
		if(!p.moveElement(imageName, x, y)) {
			...
			p.addImage(imageName,15, frame, x, y, true, img, zoom);
		}
		String buttonName = name + "_button";
		if(!p.moveElement(buttonName, x, y)) {
			p.addButton(buttonName, 15, frame,  x, y, wid, hei, code, true);
